  ...
  return (
    <>
      <div id='who' ref={context.refabout} className='m-3'>
        <Typography  sx={{fontSize:{sm:'35px',xs:'25px'},color:context.dark?'white':'black'}}>who cat shop
        </Typography>
        <div className='md:w-[50%] w-[95%] sm:w-[85%] md:text-[18px] text-[14px] m-3 text-zinc-500 dark:text-zinc-300'>
      ...
      <section className="sm:h-[400px] h-[auto] flex flex-column my-[150px]">
        <h2 className="text-center" style={{ color: context.dark ? 'rgb(255, 255, 255)' : 'rgb(66, 66, 66)' }}>Overview</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 px-4 gap-5 mt-5 mx-auto" style={{ marginTop: '-20px' }}>
          {features.map((item) => (
            <motion.div key={item.title.length} className="  mt-1 shadow-lg rounded-3 p-3 bg-zinc-100 dark:bg-zinc-800" initial={{ y: -100, opacity: 0 }} transition={{ delay: 1, type: 'spring', damping: 20 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }}  >
              <div className='flex'>
                <p className="w-[35px] h-[35px] bg-zinc-700 flex items-center justify-content-center text-light rounded-full text-1xl">
                  {item.icon}</p>
                <h6 className="ms-3 mt-2 text-zinc-900 dark:text-zinc-100">{item.title}</h6>
              </div>

              <p className="ms-3 text-blue-900 dark:text-blue-500">{item.content}</p>
            </motion.div>
